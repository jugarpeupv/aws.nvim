...
local function hint_line()
  ...
  local hints = {}
  if km.open_menu then
    table.insert(hints, km.open_menu .. " menu")
  end
  if km.delete then
...
local function render(buf, st)
  ...
  local lines = {
    ...
    hint_line(),
    sep,
    pad_right("Name", name_width) .. pad_right("Status", status_width) .. pad_right("Class", class_width) .. pad_right(
      "Billing",
      billing_width
    ) .. pad_right("Items", 10) .. "Size",
    sep,
  }
  ...
  for _, item in ipairs(st.items) do
    ...
    local class = item.TableClassSummary and (item.TableClassSummary.TableClass or "STANDARD") or "STANDARD"
...
local function fetch(buf, st, call_opts)
  ...
  local function describe_all(names)
    ...
    for _, name in ipairs(names) do
      spawn.run({ "dynamodb", "describe-table", "--table-name", name, "--output", "json" }, function(ok, lines)
        if desc_gen ~= st.fetch_gen then
          return
        end
        if ok then
  ...
  local function list_page(last_name)
    ...
    spawn.run(args, function(ok, lines)
      ...
      local token = type(data.LastEvaluatedTableName) == "string" and data.LastEvaluatedTableName or nil
      if token then
        list_page(token)
      else
...
function M.open(call_opts)
  ...
  local function delete_visual(r1, r2)
    ...
    local label = #names == 1 and ("Yes, delete " .. names[1]) or ("Yes, delete " .. #names .. " tables")
    vim.ui.select({ label, "Cancel" }, { prompt = "Delete DynamoDB tables?" }, function(_, idx)
