...
local function render(buf, table_name)
  ...
  row("Created",            tbl.CreationDateTime and
    tostring(tbl.CreationDateTime) or "—")
  row("Item Count",         tostring(tbl.ItemCount or 0))
  row("Table Size",         fmt_bytes(tbl.TableSizeBytes))
  local class = tbl.TableClassSummary
    and (tbl.TableClassSummary.TableClass or "STANDARD") or "STANDARD"
  if class == "STANDARD_INFREQUENT_ACCESS" then class = "STANDARD_INFREQUENT_ACCESS (IA)" end
  ...
  if #gsis > 0 then
    ...
    for _, gsi in ipairs(gsis) do
      ...
      if gsi.ProvisionedThroughput then
        ...
        row("    Read / Write",
          tostring(pt.ReadCapacityUnits or 0) .. " RCU  /  "
          .. tostring(pt.WriteCapacityUnits or 0) .. " WCU")
      end
  ...
  if tbl.PointInTimeRecoveryDescription then
    ...
    row("  Earliest Restore",pitr.EarliestRestorableDateTime and
      tostring(pitr.EarliestRestorableDateTime) or "—")
    row("  Latest Restore",  pitr.LatestRestorableDateTime and
      tostring(pitr.LatestRestorableDateTime) or "—")
  end
