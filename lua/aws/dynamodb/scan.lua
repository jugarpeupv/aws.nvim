...
local _results_line = {} -- luacheck: ignore 241

local function buf_name(tn)
  return "aws://dynamodb/scan/" .. tn
end
...
local function build_eav(st, filter_eav)
  ...
  for k, v in pairs(filter_eav) do
    eav[k] = v
  end
  if not next(eav) then
    return nil
  end
...
local function build_ean(st, filter_ean)
  ...
  for k, v in pairs(filter_ean) do
    ean[k] = v
  end
  if not next(ean) then
    return nil
  end
...
local function build_kce(st)
  if not st.pk_name or not st.pk_value then
    return nil
  end
  local expr = "#pk = :pk"
...
local function build_args(st, last_key)
  local filter_expr, filter_ean, filter_eav = ss.build_filter_expression(st.filters)
...
local function fetch(table_name, buf, call_opts, last_key)
  ...
  spawn.run(args, function(ok, lines)
    ...
    st.last_key = type(data.NextToken) == "string" and data.NextToken
      or (type(data.LastEvaluatedKey) == "table" and vim.json.encode(data.LastEvaluatedKey) or nil)
    render(buf, st)
  end, call_opts)
end
...
function M.open(table_name, call_opts)
  ...
  if not _state[table_name] then
    _state[table_name] = ss.new_state(table_name, config.resolve_region(call_opts), config.resolve_profile(call_opts))
  end
