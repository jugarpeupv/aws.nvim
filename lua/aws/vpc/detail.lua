...
local function render(buf, vpc_id)
  ...
  if st.vpc then
    ...
    if type(v.CidrBlockAssociationSet) == "table" then
      for _, a in ipairs(v.CidrBlockAssociationSet) do
        if a.CidrBlock then
          table.insert(cidrs, a.CidrBlock)
        end
      end
    end
    if #cidrs == 0 and v.CidrBlock then
      cidrs = { v.CidrBlock }
    end
  ...
  if st.vpc then
    for _, l in ipairs(fmt_tags(st.vpc.Tags)) do
      table.insert(lines, l)
    end
  elseif st.vpc == false then
    table.insert(lines, "  (error)")
...
local function fetch(vpc_id, buf, call_opts)
  ...
  spawn.run({ "ec2", "describe-vpcs", "--vpc-ids", vpc_id, "--output", "json" }, function(ok, out)
    local st = _state[vpc_id]
    if ok then
      local ok2, data = pcall(vim.json.decode, table.concat(out, "\n"))
      st.vpc = (ok2 and type(data) == "table" and type(data.Vpcs) == "table" and data.Vpcs[1]) or false
    else
      st.vpc = false
    end
    render(buf, vpc_id)
