...
local function buf_name(id) return "aws://vpc/detail/route_table/" .. id end
...
local function render(buf, id)
  ...
  for _, l in ipairs(section("Routes")) do table.insert(lines, l) end
  if st.rt then
    local routes = {}
  ...
  for _, l in ipairs(section("Subnet Associations")) do table.insert(lines, l) end
  if st.rt then
    local subs = {}
...
local function fetch(id, vpc_id, buf, call_opts)
  ...
  local args = { "ec2", "describe-route-tables",
    "--route-table-ids", id,
    "--output", "json" }
  spawn.run(args, function(ok, out)
    ...
    if ok then
      ...
      st.rt = (ok2 and type(data) == "table"
        and type(data.RouteTables) == "table"
        and data.RouteTables[1]) or false
    else
      st.rt = false
    end
    render(buf, id)
...
function M.open(id, vpc_id, call_opts)
  ...
  keymaps.apply_vpc_section(buf, {
    refresh = function() fetch(id, vpc_id, buf, call_opts) end,
  })
