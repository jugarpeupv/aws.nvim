...
local function render(buf, vpc_id)
  ...
  local title = "VPC  Route Tables:  " .. vpc_id .. (st.fetching and "   [loading…]" or "")
  ...
  if #items == 0 and not st.fetching then
    ...
    for _, rt in ipairs(items) do
      local assoc_str = #rt.associations > 0 and table.concat(rt.associations, ", ") or "(main)"
      table.insert(lines, string.format(fmt, rt.id, rt.name ~= rt.id and rt.name or "—", assoc_str))
...
local function fetch(vpc_id, buf, call_opts)
  ...
  spawn.run(
    { "ec2", "describe-route-tables", "--filters", "Name=vpc-id,Values=" .. vpc_id, "--output", "json" },
    function(ok, out)
      ...
      for _, rt in ipairs(data.RouteTables) do
        ...
        if type(rt.Associations) == "table" then
          for _, a in ipairs(rt.Associations) do
            if a.SubnetId then
              table.insert(assoc_subnets, a.SubnetId)
            end
          end
        end
        table.insert(result, {
