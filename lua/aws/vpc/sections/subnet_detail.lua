...
local function buf_name(id) return "aws://vpc/detail/subnet/" .. id end
...
local function fetch(id, buf, call_opts)
  ...
  spawn.run(
    { "ec2", "describe-subnets", "--subnet-ids", id, "--output", "json" },
    function(ok, out)
      local st = _state[id]
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(out, "\n"))
        st.subnet = (ok2 and type(data) == "table"
          and type(data.Subnets) == "table"
          and data.Subnets[1]) or false
      else
        st.subnet = false
      end
      render(buf, id)
...
function M.open(id, call_opts)
  ...
  keymaps.apply_vpc_section(buf, {
    refresh = function() fetch(id, buf, call_opts) end,
  })
