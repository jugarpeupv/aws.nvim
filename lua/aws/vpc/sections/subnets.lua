...
local function render(buf, vpc_id)
  ...
  if #visible == 0 and not st.fetching then
    ...
    local fmt = string.format("  %%-%ds  %%-%ds  %%-%ds  %%-%ds  %%-5s  %%s", w_name, w_id, w_cidr, w_az)
    ...
    for _, s in ipairs(visible) do
      table.insert(
        lines,
        string.format(fmt, s.name, s.id, s.cidr, s.az, tostring(s.available_ips), s.map_public_ip and "yes" or "no")
      )
...
local function fetch(vpc_id, buf, call_opts)
  ...
  spawn.run(
    { "ec2", "describe-subnets", "--filters", "Name=vpc-id,Values=" .. vpc_id, "--output", "json" },
    function(ok, out)
