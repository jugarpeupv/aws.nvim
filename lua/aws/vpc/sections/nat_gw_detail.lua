...
local function buf_name(id) return "aws://vpc/detail/nat_gw/" .. id end
  ...
  spawn.run(
    ...
    function(ok, out)
      ...
      if ok then
        ...
        st.nat = (ok2 and type(data) == "table"
          and type(data.NatGateways) == "table"
          and data.NatGateways[1]) or false
      else
        st.nat = false
      end
      render(buf, id)
