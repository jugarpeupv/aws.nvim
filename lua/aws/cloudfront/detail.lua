...
local function origin_type(origin)
  if type(origin.S3OriginConfig) == "table" then
    return "S3"
  end
  if type(origin.CustomOriginConfig) == "table" then
    return "Custom"
  end
  return "Unknown"
end
...
local function render(buf, id)
  ...
  if km.detail_invalidate then
    table.insert(hints, km.detail_invalidate .. " invalidate")
  end
  if #hints > 0 then
  ...
  if #origin_items > 0 then
    ...
    for i, o in ipairs(origin_items) do
      ...
      if type(o.CustomOriginConfig) == "table" then
        ...
        if type(coc.OriginSSLProtocols) == "table" and type(coc.OriginSSLProtocols.Items) == "table" then
  ...
  if dcb then
    ...
    -- TTLs
    if dcb.DefaultTTL then
      row("  Default TTL (s)", tostring(dcb.DefaultTTL))
    end
    if dcb.MinTTL then
      row("  Min TTL (s)", tostring(dcb.MinTTL))
    end
    if dcb.MaxTTL then
      row("  Max TTL (s)", tostring(dcb.MaxTTL))
    end
  ...
  if #cb_items > 0 then
    ...
    for i, cb in ipairs(cb_items) do
      ...
      if cb.DefaultTTL then
        row("  Default TTL (s)", tostring(cb.DefaultTTL))
      end
    end
  end
