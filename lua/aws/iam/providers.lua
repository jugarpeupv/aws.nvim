...
local function merge(oidc, saml)
  ...
  for _, v in ipairs(saml) do
    table.insert(out, { arn = v.Arn, kind = "SAML", name = v.SAMLMetadataDocument and v.Arn:match("/([^/]+)$") or v.Arn:match("/([^/]+)$") })
  end
  return out
end
...
local function render(buf, st)
  ...
  if not next(st.line_map) then table.insert(lines, "(no providers match)") end
...
local function fetch(buf, st, call_opts)
  ...
  spawn.run({ "iam", "list-open-id-connect-providers", "--output", "json" }, function(ok, lines)
    ...
    if ok then
      ...
      if ok2 and type(data) == "table" then
        oidc_list = type(data.OpenIDConnectProviderList) == "table"
          and data.OpenIDConnectProviderList or {}
      end
    end
    on_done()
  end, call_opts)
  ...
  spawn.run({ "iam", "list-saml-providers", "--output", "json" }, function(ok, lines)
    ...
    if ok then
      ...
      local ok2, data = pcall(vim.json.decode, raw)
      if ok2 and type(data) == "table" then
        saml_list = type(data.SAMLProviderList) == "table"
          and data.SAMLProviderList or {}
      end
    end
    on_done()
  end, call_opts)
...
function M.open(call_opts)
  ...
  keymaps.apply_iam_list(buf, {
    open_detail = function()
      ...
      if not key then vim.notify("aws.nvim: no provider under cursor", vim.log.levels.WARN); return end
