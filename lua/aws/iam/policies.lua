...
local function hint_line()
  ...
  if km.toggle_scope       then table.insert(parts, km.toggle_scope       .. " scope")  end
...
local function render(buf, st)
  ...
  for _, item in ipairs(st.items) do
    ...
    local a = item.Arn        or ""
    if st.filter == "" or n:lower():find(st.filter:lower(), 1, true)
      or a:lower():find(st.filter:lower(), 1, true) then
      local nw = vim.fn.strdisplaywidth(n)
      local aw = vim.fn.strdisplaywidth(a)
      if nw > name_w then name_w = nw end
      if aw > arn_w  then arn_w  = aw end
    end
  end
  name_w = name_w + 2
  arn_w  = arn_w  + 2
  ...
  local title = "IAM  Policies  " .. scope_label
    .. "   [region: " .. st.region .. "]"
  ...
  for _, item in ipairs(st.items) do
    ...
    if st.filter == "" or name:lower():find(st.filter:lower(), 1, true)
      or arn:lower():find(st.filter:lower(), 1, true) then
      table.insert(lines,
        pad_right(name,    name_w) .. "  "
        .. pad_right(updated, date_w) .. "  "
        .. arn
  ...
  if not next(st.line_map) then table.insert(lines, "(no policies match)") end
...
local function fetch(buf, st, call_opts)
  ...
  local function fetch_page(marker)
    ...
    spawn.run(args, function(ok, lines)
      ...
      st.items    = all
      render(buf, st)
      if next_marker then fetch_page(next_marker)
      else st.cache[scope] = all end
    end, call_opts)
  end
...
function M.open(call_opts)
  ...
  if not _state[identity] then
    _state[identity] = {
      items = {}, filter = "", scope = "Local", line_map = {}, cache = {},
  ...
  keymaps.apply_iam_list(buf, {
    open_detail = function()
      ...
      if not arn then vim.notify("aws.nvim: no policy under cursor", vim.log.levels.WARN); return end
    ...
    filter = function()
      vim.ui.input({ prompt = "Filter policies: ", default = st.filter }, function(input)
        ...
        st.filter = input
        if st.cache[st.scope] then st.items = st.cache[st.scope] end
        render(buf, st)
      end)
    end,
    clear_filter = function()
      st.filter = ""
      if st.cache[st.scope] then st.items = st.cache[st.scope]; render(buf, st)
