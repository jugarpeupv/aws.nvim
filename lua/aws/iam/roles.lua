...
local function render(buf, st)
  ...
  local lines = {
    ...
    hint_line(),
    sep,
    pad_right("Name", name_w) .. "  " .. pad_right("Created", date_w) .. "  " .. "Description",
    sep,
  }
  ...
  for _, item in ipairs(st.items) do
    ...
    if st.filter == "" or name:lower():find(st.filter:lower(), 1, true) then
      table.insert(lines, pad_right(name, name_w) .. "  " .. pad_right(created, date_w) .. "  " .. desc)
      st.line_map[#lines] = name
    end
  end

  if not next(st.line_map) then
    table.insert(lines, "(no roles match)")
  end
...
function M.open(call_opts)
  ...
  keymaps.apply_iam_list(buf, {
    open_detail = function()
      ...
      if not name then
        vim.notify("aws.nvim: no role under cursor", vim.log.levels.WARN)
        return
      end
