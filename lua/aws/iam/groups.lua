...
local function render(buf, st)
  ...
  local lines = {
    ...
    hint_line(),
    sep,
    pad_right("Name", name_w) .. "  " .. pad_right("Created", date_w) .. "  " .. pad_right("Path", path_w),
    sep,
  }
  ...
  if not next(st.line_map) then
    table.insert(lines, "(no groups match)")
  end
...
function M.open(call_opts)
  ...
  keymaps.apply_iam_list(buf, {
    open_detail = function()
      ...
      if not name then
        vim.notify("aws.nvim: no group under cursor", vim.log.levels.WARN)
        return
      end
