...
local function render(buf, arn)
  ...
  -- Versions
  for _, l in ipairs(section("Versions")) do table.insert(lines, l) end
  if st.versions then
    if #st.versions == 0 then table.insert(lines, "  (none)")
    else
  ...
  for _, l in ipairs(section("Attached To")) do table.insert(lines, l) end
  if st.entities then
    local any = false
...
local function fetch(arn, buf, call_opts)
  ...
  spawn.run({ "iam", "get-policy", "--policy-arn", arn, "--output", "json" },
    function(ok, lines)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
        st.policy = (ok2 and type(data) == "table" and data.Policy) or false
      else st.policy = false end
      on_done()
    end, call_opts)

  spawn.run({ "iam", "list-policy-versions", "--policy-arn", arn, "--output", "json" },
    function(ok, lines)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
        st.versions = (ok2 and type(data) == "table" and type(data.Versions) == "table" and data.Versions) or {}
      else st.versions = false end
      on_done()
    end, call_opts)

  spawn.run({ "iam", "list-entities-for-policy", "--policy-arn", arn, "--output", "json" },
    function(ok, lines)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
