...
local function json_lines(val, indent)
  ...
  elseif t == "table" then
    ...
    if is_array then
      if #val == 0 then return { pad .. "[]" } end
      local out = { pad .. "[" }
      for i, v in ipairs(val) do
        local child = json_lines(v, indent + 1)
        if i < #val then child[#child] = child[#child] .. "," end
        for _, l in ipairs(child) do table.insert(out, l) end
      end
      ...
      for i, k in ipairs(keys) do
        ...
        if i < #keys then inner[#inner] = inner[#inner] .. "," end
        for _, l in ipairs(inner) do table.insert(out, l) end
      end
      table.insert(out, pad .. "}")
      return out
    end
...
local function render(buf, name)
  ...
  for _, l in ipairs(section("Trust Policy (AssumeRole)")) do table.insert(lines, l) end
  if st.role then
  ...
  if st.attached then
    if #st.attached == 0 then table.insert(lines, "  (none)")
    else
  ...
  if st.inline then
    if #st.inline == 0 then table.insert(lines, "  (none)")
    else
      for _, n in ipairs(st.inline) do table.insert(lines, "  " .. n) end
    end
  else
  ...
  -- Last Accessed
  for _, l in ipairs(section("Last Accessed")) do table.insert(lines, l) end
  ...
  elseif #st.last_accessed == 0 then
    ...
    table.insert(lines, string.format(
      "  %-50s  %-26s  %s", "Service", "Last Authenticated", "Region"))
...
local function fetch(name, buf, call_opts)
  ...
  spawn.run({ "iam", "get-role", "--role-name", name, "--output", "json" },
    function(ok, lines_out)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines_out, "\n"))
        if ok2 and type(data) == "table" and type(data.Role) == "table" then
          ...
          if type(doc) == "string" then
            ...
            if ok3 then r.AssumeRolePolicyDocument = decoded end
          end
          st.role = r
  ...
  spawn.run({ "iam", "list-attached-role-policies", "--role-name", name, "--output", "json" },
    function(ok, lines_out)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines_out, "\n"))
        st.attached = (ok2 and type(data) == "table"
          and type(data.AttachedPolicies) == "table"
          and data.AttachedPolicies) or {}
      else st.attached = false end
      on_done()
    end, call_opts)

  spawn.run({ "iam", "list-role-policies", "--role-name", name, "--output", "json" },
    function(ok, lines_out)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines_out, "\n"))
        st.inline = (ok2 and type(data) == "table"
          and type(data.PolicyNames) == "table"
          and data.PolicyNames) or {}
      else st.inline = false end
      on_done()
    end, call_opts)
end
...
function M.open(name, call_opts)
  ...
  keymaps.apply_iam_detail(buf, {
    refresh = function() fetch(name, buf, call_opts) end,
  })
