...
local function render(buf, username)
  ...
  -- Groups
  for _, l in ipairs(section("Groups")) do
    table.insert(lines, l)
  end
  if st.groups then
    if #st.groups == 0 then
  ...
  -- Access Keys
  for _, l in ipairs(section("Access Keys")) do
    table.insert(lines, l)
  end
  ...
  -- MFA Devices
  for _, l in ipairs(section("MFA Devices")) do
    table.insert(lines, l)
  end
  if st.mfa then
    if #st.mfa == 0 then
...
local function fetch(username, buf, call_opts)
  ...
  -- get-user
  spawn.run({ "iam", "get-user", "--user-name", username, "--output", "json" }, function(ok, lines)
    if ok then
  ...
  spawn.run({ "iam", "list-groups-for-user", "--user-name", username, "--output", "json" }, function(ok, lines)
    if ok then
      local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
  ...
  spawn.run({ "iam", "list-attached-user-policies", "--user-name", username, "--output", "json" }, function(ok, lines)
    if ok then
      local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
  ...
  spawn.run({ "iam", "list-user-policies", "--user-name", username, "--output", "json" }, function(ok, lines)
    if ok then
      local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
  ...
  spawn.run({ "iam", "list-access-keys", "--user-name", username, "--output", "json" }, function(ok, lines)
    if ok then
      local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
      st.access_keys = (
        ok2
        and type(data) == "table"
        and type(data.AccessKeyMetadata) == "table"
        and data.AccessKeyMetadata
      ) or {}
    else
  ...
  spawn.run({ "iam", "list-mfa-devices", "--user-name", username, "--output", "json" }, function(ok, lines)
    if ok then
      local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
