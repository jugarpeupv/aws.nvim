...
local function render(buf, arn)
  ...
  if st.kind == "OIDC" then
    for _, l in ipairs(section("OIDC Provider")) do table.insert(lines, l) end
    if st.data then
      table.insert(lines, kv("ARN",         arn))
      table.insert(lines, kv("URL",         st.data.Url))
      table.insert(lines, kv("Created",     st.data.CreateDate))
      -- Client IDs
      for _, l in ipairs(section("Client IDs")) do table.insert(lines, l) end
      local cids = type(st.data.ClientIDList) == "table" and st.data.ClientIDList or {}
      if #cids == 0 then table.insert(lines, "  (none)")
      else for _, id in ipairs(cids) do table.insert(lines, "  " .. id) end end
      -- Thumbprints
      for _, l in ipairs(section("Thumbprints")) do table.insert(lines, l) end
      local tps = type(st.data.ThumbprintList) == "table" and st.data.ThumbprintList or {}
      if #tps == 0 then table.insert(lines, "  (none)")
      else for _, tp in ipairs(tps) do table.insert(lines, "  " .. tp) end end
    else
  ...
  elseif st.kind == "SAML" then
    for _, l in ipairs(section("SAML Provider")) do table.insert(lines, l) end
    if st.data then
      table.insert(lines, kv("ARN",         arn))
      ...
      if doc then
        ...
        for line in (doc .. "\n"):gmatch("([^\n]*)\n") do
          ...
          n = n + 1
          if n >= 10 then table.insert(lines, "  … (truncated)"); break end
        end
      end
    else
      table.insert(lines, st.data == false and "  (error)" or "  [loading…]")
...
function M.open(arn, kind, call_opts)
  ...
  keymaps.apply_iam_detail(buf, {
    refresh = function() fetch(arn, kind, buf, call_opts) end,
  })
