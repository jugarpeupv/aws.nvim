...
local function render(buf, name)
  ...
  -- Members
  for _, l in ipairs(section("Members")) do table.insert(lines, l) end
  if st.members then
    if #st.members == 0 then table.insert(lines, "  (none)")
    else
...
local function fetch(name, buf, call_opts)
  ...
  spawn.run({ "iam", "get-group", "--group-name", name, "--output", "json" },
    function(ok, lines)
      if ok then
        ...
        if ok2 and type(data) == "table" then
          ...
          st.group = false; st.members = false
        end
      else
        st.group = false; st.members = false
      end
  ...
  spawn.run({ "iam", "list-attached-group-policies", "--group-name", name, "--output", "json" },
    function(ok, lines)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
  ...
  spawn.run({ "iam", "list-group-policies", "--group-name", name, "--output", "json" },
    function(ok, lines)
      if ok then
        local ok2, data = pcall(vim.json.decode, table.concat(lines, "\n"))
